...
public class Main {
    ...
    public static void countTueWedThur(List<Entry> entries) {
        ...
        long count1 = 0;
        // write stream
        entries.stream().filter(entry -> entry.getDay().equals(Day.TUESDAY) || entry.getDay().equals(Day.WEDNESDAY)
    ...
    public static void weekendList(List<Entry> entries) {
        ...
        weekends = entries.stream().filter(entry -> entry.getDay().equals(Day.SATURDAY) ||
        entry.getDay().equals(Day.SUNDAY)).collect(Collectors.toList());
    ...
    public static void weekdaySet(List<Entry> entries) {
        ...
        weekdays = entries.stream().filter(entry -> entry.getDay() != (Day.SATURDAY) ||
                entry.getDay() != (Day.SUNDAY))
    ...
    public static void printDurationGreaterThan10(List<Entry> entries){
        ...
        // write stream
        entries.stream().filter(entry -> entry.getDuration() > 10)
                .forEach(entry -> System.out.println(entry));
    ...
    public static void findMaxDuration(List<Entry> entries){
        ...
        temp = entries.stream().mapToInt(entry -> entry.getDuration())
                .max().getAsInt();
    ...
    public static void listGreaterThan50(List<Entry> entries){
        ...
        System.out.println("Stream, filter, collect:");
        // write stream
        entries.stream().filter(entry -> entry.getDuration() > 50)
